-- Task mutations write a task_events row in the same transaction.
-- SECURITY INVOKER: RLS on tasks still applies. Never service-role.
...
begin
  insert into public.tasks (
    ...
    due_date, priority, visibility, created_by, status
  ) values (
    p_title,
    p_notes,
    p_team_id,
...
begin
  ...
  update public.tasks set
    ...
    team_id = coalesce((p_patch ->> 'team_id')::uuid, team_id),
...
revoke all on function public.create_task_with_event(text, text, uuid, text, uuid, uuid, date, public.task_priority, public.task_visibility, text) from public;
revoke all on function public.update_task_with_event(uuid, jsonb, text) from public;
grant execute on function public.create_task_with_event(text, text, uuid, text, uuid, uuid, date, public.task_priority, public.task_visibility, text) to authenticated;
