...
export async function TareasBoard({
  searchParams,
}: {
  searchParams: Promise<TareasSearch> | TareasSearch;
}) {
      ...
      {filtered.length === 0 ? (
        ...
        <div className="px-4 pb-10 md:px-7">
          <div className="overflow-hidden rounded-[14px] border border-line bg-sheet">
            ...
            {overdue.length > 0
              ? overdue.map((task) => <TaskRow key={task.id} task={task} href={`/tareas/${task.id}`} />)
              : null}
            {[...dated.entries()]
